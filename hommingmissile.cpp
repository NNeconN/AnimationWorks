...
// 弾更新
void HommingMissileMgr::Update(XMFLOAT4X4 mtx,XMFLOAT3 target) {
	...
	// 全弾更新
	for (auto& b : m_hominggmissiles) {
		b->SetTarget(target);
		b->Update();
	}

	// 死んでる弾削除
	auto it = m_hominggmissiles.begin();
	while (it != m_hominggmissiles.end()) {

		if ((*it)->isLive() == false) {
			it = m_hominggmissiles.erase(it);
		}
		else ++it;
	}
}
...
// 弾発射
void HommingMissileMgr::Fire(XMFLOAT4X4 mtx, DirectX::XMFLOAT3 target) {
	...
	XMFLOAT3 vel = XMFLOAT3(mtx._31*2.0f, mtx._32*2.0f, mtx._33*2.0f);
	p->SetVelocity(vel);
	p->SetInitailQuaternion(mtx);
	p->SetTarget(target);

	// 最後尾に弾を追加
	m_hominggmissiles.emplace_back(std::move(p));
}
